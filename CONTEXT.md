...
**网关 (Gateway)**:
运行时访问工具面的唯一聚合接入点：装配工具面——内置 DuckDB 工具直列，外部 MCP 工具经固定发现面按需可见（agent 搜索目录、按句柄调用；见 ADR-0105），并在此强制分级审批（ADR-0080）、审计、物化命名（ADR-0077）。内置运行时在 app 进程内直接经网关路由；外部运行时经**桥接**回连网关。网关是强制边界——运行时绕过网关直连工具，则审批/审计/物化皆不可强制。
