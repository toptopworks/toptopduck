...
**网关 (Gateway)**:
运行时访问工具面的唯一聚合接入点：装配工具面——内置 DuckDB 工具与用户注册的 CLI 工具直列，启用服务器的外部 MCP 工具经固定发现面按需可见（agent 搜索目录、按句柄调用；见 ADR-0105/0106），并在此强制分级审批（ADR-0080）、审计、物化命名（ADR-0077）。内置运行时在 app 进程内直接经网关路由；外部运行时经**桥接**回连网关。网关是强制边界——运行时绕过网关直连工具，则审批/审计/物化皆不可强制。
...
**工具 (Tool)**:
运行时在轮次内可调用的、带名称与参数 schema 的能力；所有调用经 app 网关统一路由（可审计、可限、可沙箱）。**内置工具**作用于工作集：探索查询（scratch，轮末即弃）与显式物化（晋升为中间结果、得 result_N 名、受上限约束）；**外部工具**来自用户配置的外部工具源——MCP 服务器与 CLI 工具（见「CLI 工具」），审批 / 审计 / 轨迹语义不因配置来源而异。工具调用是轮次的轨迹构成，**非轮次本身**，不推进计步序。
_Avoid_: 命令(command)、扩展(extension)、接口(interface)

**技能 (Skill)**:
可挂载到会话的命名能力包 = 提示片段（Markdown 正文）+ 可选的外部工具引用（MCP 服务器或 CLI 工具），遵循 [Agent Skills 规范](https://agentskills.io/specification)：一技能 = 一目录 + `SKILL.md`（YAML frontmatter + Markdown 正文），`name`（kebab-case、≤64、等于目录名）即其稳定身份。技能住全局技能库（单一注册表），每会话可多挂载、中途增减；由 app 在轮次装配阶段统一注入（提示片段进系统提示、工具引用为声明性元数据——所指者须在配置中配置并启用方可用，引用不改变配置/启用状态），与运行时无涉——改变 agent 的「答法」，不动工作集与 recipe 结构。
...
**CLI 工具 (CLI Tool)**:
用户注册的具名**非交互命令工具**，与 MCP 服务器并列的外部工具第二来源（ADR-0108）：注册项声明可执行程序、参数与描述，由网关代为拉起子进程执行——直接 argv、不经 shell 解释器，无交互、无 shell 语法，只支持非交互批处理；代码解释器是「参数恰为自由代码」的退化解，非独立机制。注册 / 启用是机器级、与 MCP 服务器同轴，空注册集是合法状态。工具级语义与 MCP 外部工具同构——分级审批、审计、调用入轮次执行轨迹；呈现上**直列**进工具面（数量少、显式注册、稳定存在——不同于 MCP 目录的固定发现面）。
