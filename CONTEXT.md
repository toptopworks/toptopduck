...
**轮次 (Turn)**:
一次完整的交互单元 = 一次提问 + 一个 outcome（产出中间结果只是其中一种）。轮次恒在对话 thread 中**可见**——无论 outcome 是否产出中间结果，条目本身始终存在；产不产中间结果只决定 outcome 类型，与计步序、是否进对话窗口是相互独立的维度。对话 thread 还含**源生命周期事件**（加/换/删源）——与轮次并列、恒可见、占时序位置，但**非轮次**、不进 LLM 窗口（ADR-0040）。
