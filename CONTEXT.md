...
**网关 (Gateway)**:
运行时访问工具面的唯一聚合接入点：装配工具面——内置 DuckDB 工具与注册 CLI 工具直列，启用服务器的外部 MCP 工具经固定发现面按需可见（agent 搜索目录、按句柄调用；见 ADR-0105/0106），并在此强制分级审批（ADR-0080）、审计、物化命名（ADR-0077）。内置运行时在 app 进程内直接经网关路由；外部运行时经**桥接**回连网关。网关是强制边界——运行时绕过网关直连工具，则审批/审计/物化皆不可强制。
...
**技能 (Skill)**:
可挂载到会话的命名能力包 = 提示片段（Markdown 正文）+ 可选的外部工具引用（MCP 服务器或 CLI 工具），遵循 [Agent Skills 规范](https://agentskills.io/specification)：一技能 = 一目录 + `SKILL.md`（YAML frontmatter + Markdown 正文），`name`（kebab-case、≤64、等于目录名）即其稳定身份。技能住全局技能库（单一注册表），来源二分——用户（创建 / 导入）与内置（app 自撰、随版发行、配套内置 CLI 注册项，对应项检测到且启用时自动进入新会话活跃集，见 ADR-0109）；每会话可多挂载、中途增减；由 app 在轮次装配阶段统一注入（提示片段进系统提示、工具引用为声明性元数据——所指者须在配置中配置并启用方可用，引用不改变配置/启用状态），与运行时无涉——改变 agent 的「答法」，不动工作集与 recipe 结构。
...
随 app 版本发行的 **CLI 注册项**（ADR-0109）：定义（可执行程序、参数与描述、env）由 app 策展，app 版本更新按基线对账——未编辑条目随版升级，编辑过原样保留，可显式恢复内置定义。本地安装检测（存在判定，零副作用）命中即自动注册并默认启用——开箱即用；未检测到的条目休眠——不进工具面、不进审批面；检测态不影响调用时解析兜底（探针语义，见 ADR-0108）。**不可删除**，禁用是唯一关停轴（禁用为绝对权威，见 ADR-0106）；可编辑，唯 `name` 锁定——名字是稳定身份，技能引用与审批信任都挂它。配套**内置技能**（app 自撰、随版发行）于本条目检测到 + 启用时自动进入新会话活跃技能集（见「技能」）。
