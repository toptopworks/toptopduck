...
**上次模型姿势**:
新会话起步时外部运行时沿用的模型 + 思考强度，= 该适配器最近一次成功选择的姿势（ADR-0100）。按适配器分记、住 app-config；回填即已选（turn 注入）——「默认（推荐）」仅在从未选择或已清除时出现，清除行置空该适配器条目。条目悬空（适配器未检测到 / 目录不含该模型）时保留，重探测后自动复效。与默认运行时的分工：运行时起步是显式偏好（用户在设置页设定），模型姿势起步是自动回填（记录行为痕迹）；会话内改选即更新条目，不回写默认运行时。
