...
新会话的起步运行时，机器级执行偏好（内置 或 某一外部适配器），住 app-config、缺省内置（ADR-0098）。会话内切换运行时不回写默认；resume 不回落到它——延续会话最后运行时。起步时解析：默认指向的适配器不再被检测到时，该次起步诚实降级为内置，偏好不销毁。活跃接入档案仅在内置运行时执行时被读取——「激活」是接入面的属性，与运行时起步无关。
