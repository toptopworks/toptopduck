...
pub(crate) const PROGRESS_POLL_INTERVAL: Duration = Duration::from_millis(25);

pub(crate) struct ProgressClock {
    /// The cap the clock arms with (the ADR-0021-aligned 120 s in
    /// production; short values ride the `with_caps` test seam).
    timeout: Duration,
    /// The next killable instant, in UNIX millis. Re-armed by `touch`,
    /// deferred by the watchdog while frozen.
    deadline_ms: AtomicU64,
    /// Nested freeze depth: a segment waiting on an external principal
    /// (gateway dispatch, approval condvar, permission decision, an open
    /// tool-call window) holds a [`FreezeGuard`]; the watchdog defers the
    /// deadline while this is > 0.
    frozen: AtomicU32,
...
impl ProgressClock {
    ...
    pub(crate) fn arm(
        ...
        let clock = Arc::new(Self {
            timeout,
            deadline_ms: AtomicU64::new(now_ms() + timeout.as_millis() as u64),
        ...
        thread::spawn(move || {
            ...
            // panicking watchdog is logged, never silently eaten.
            let drove = catch_unwind(AssertUnwindSafe(|| {
                while let Some(clock) = weak.upgrade() {
                    ...
                    if clock.frozen.load(Ordering::SeqCst) > 0 {
                        ...
                        clock.defer(timeout);
                    } else if now_ms() >= clock.deadline_ms.load(Ordering::SeqCst) {
    ...
    /// otherwise a user / close cancel. `cap` rides the caller's configured
    /// wall clock (`None` between turns and on the `with_caps(None)` seam,
    /// where the default degenerates to zero -- a test-only shape).
    pub(crate) fn cancel_landing(clock: Option<&Arc<Self>>, cap: Option<Duration>) -> Termination {
        match clock {
            Some(clock) if clock.is_timed_out() => Termination::NoProgress(cap.unwrap_or_default()),
    ...
    /// is safe (a permission decision inside an open tool-call window);
...
mod tests {
    ...
    /// generation, and the expiry's request_if cannot fire.
    #[test]
    fn expiry_after_a_turn_boundary_stands_down() {
        let (token, _clock, generation) = armed(CAP);
