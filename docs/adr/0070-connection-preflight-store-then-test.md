...
校准：上列 key 流向以「key 已在 keychain」为前提，即编辑模式。添加模式下 key 缓冲于前端草稿、尚无 keychain 条目，探测因此可携**一次性显式 key** 参数——前端→Rust 单向、即用即弃、不持久化不回传（与 set key 的 one-shot 方向同构）；显式 key trim 非空则优先（keychain 不被咨询），否则回落 keychain 读取，编辑模式行为不变。
...
1. **先存后测不扩信任边界**：set key 沿用 ADR-0029 的 one-shot IPC（前端→Rust→keychain，前端只收回 bool）；test 是新增 IPC，Rust 从 keychain 取已存 key 发请求。key 流向与现状一致，编辑模式不引入「明文 key 跨 IPC 临时传递」的新路径（添加模式的一次性显式 key 见 Decision 校准——它不引入 Rust 侧 key 生命周期，与被否决的内存测试不同构）。测失败留在 keychain 的 entry 是孤儿（profile id 稳定、不引用即无害），ADR-0064 已为同型孤儿 sanctioned。
...
- **内存测试（验通过才存 key）**：前端交 key 经 IPC 给 Rust（不落 keychain，内存临时持有）→ 发测试 → 通过后用户再保存才进 keychain。心智上「验通过才存」对，但要扩一条测试专用 IPC + Rust 临时明文 key 生命周期管理，反复调参重测时 key 要么重输要么缓存；为微妙心智付双 IPC 代价不值。**否决**（否决对象是「测试专用 IPC + Rust 内存持有生命周期」的流程重构；添加模式在现有 test IPC 上附带一次性 key 参数的回落式设计不属此列——见 Decision 校准）。
...
- **校准 ADR-0064**：profile 配置流程新增 preflight 环节；model 字段选择方式从手填升级为 list models 探测下拉（探测列表与手填恒并存），profile schema 形状不变（model 仍是字段）。
