...
4. **Rail（conversation thread 列）也改为可拖拽调宽。** 镜像 sidebar resize 模式（Decision 1）：pointer 事件 hook + CSS 自定义属性注入 shell 根元素（与 sidebar 同模式）、session-body grid-template-columns 消费 + resize handle 消费 + localStorage 持久化。宽度全局共享（非 per-session）——切换 keep-alive 会话时不跳变。MIN_WIDTH 下限保护 QuestionBar toolbar 不破位（submit 按钮固定尺寸不收缩 + auth chip 文本不换行 + provider / context trigger 固定方形，窄于下限时 submit 被挤出可见区；精确 px 值留实现期）。Handle 在 workspace 折叠时隐藏（4-track grid 重新布局用于居中，边界不再映射该变量）。
...
- **Rail 宽度 per-session / 走 app-config 持久化**：per-session 导致切换会话时宽度跳变；app-config 对纯 UI 偏好过重（不需要 Rust 迁移 / 跨设备同步）。**否决**——全局 + localStorage，与 sidebar 同构。
...
- **Rail 宽度持久化走 localStorage**，与 sidebar 宽度同 store 模式、同持久化边界（设备本地、非 app-config）。
