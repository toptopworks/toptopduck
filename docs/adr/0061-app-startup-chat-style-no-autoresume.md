# 启动语义：Chat 风格（不自动 resume + 左栏全列表 + 新会话空态合并 + 不预创建实例）

## Decision

app 冷启动看到什么，定为 **Chat 风格**（如 ChatGPT）——**不自动 resume 上次会话、不预创建会话实例**，让用户主动选：
...
ADR-0034（recipe 持久化 + eager resume）+ 0035（resume 完整性校验）+ 0060（左会话栏 Chat 风格外壳）定了持久化与导航载体，但 **app 冷启动看到什么从未被任何 ADR 决策**。曾考虑「resume 上次活跃会话（`last_session_id`，app-config 记录）」，最终否决、选 Chat 风格——与 0060 刚定的 Chat 风格外壳自洽，并消解 0035 在启动路径的复杂性。

## Why

1. **Chat 风格外壳一致（ADR-0060）**——启动行为照搬 Chat 风格（不自动 resume、显示列表让用户选），比"自动恢复上次"与外壳自洽；0046 写作时未充分权衡的「2026 年 Chat 风格心智」同样适用于启动行为。
...
- **resume 上次活跃会话（`last_session_id`，初选方案）**：与 0060 Chat 风格外壳不一致 + 0035 启动路径复杂（启动撞源漂移）+ "上次活跃"定义 / quit 记录 activeSessionId / in-flight 记录等钩子纠缠。**否决**。
