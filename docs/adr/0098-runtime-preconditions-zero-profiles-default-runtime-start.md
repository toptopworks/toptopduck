...
- **被 ADR-0099 落定**：两个实施期未决项——模型按钮四态显示规格（ADR-0099 Decision 3）、默认运行时写入口形态（设置页控件，composer 无快捷写入口）。见 ADR-0099。
- **被 ADR-0100 校准**：「默认（推荐）」的锚定由未选择态收窄为从未选择或已清除（清除行置空该适配器回填条目）。见 ADR-0100。
- **被 ADR-0102 校准**：Decision 2 的 resume 半边反转——resume 恢复会话最后生效段头的运行时，不再回落默认运行时；新会话半边与「会话内切换不回写默认」不变，默认运行时的作用域收窄为新会话起步。见 ADR-0102。
