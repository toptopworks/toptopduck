...
3. **资源上限**：`memory_limit`、`threads`、结果行数上限——默认保守、用户可调（app-config 引擎默认参数；会话创建时取当前 config 作会话级快照，下发 admin 与沙箱两个执行面，恢复会话与新建同源）。
...
- 资源上限自固定编译期常量校准为「默认保守、用户可调」：常量保留为默认/回退单一来源（缺 config / 损坏 config 诚实降级到常量），用户经 app-config 调整；数值域下界 ≥1、无上界（内存上限由引擎真强制兜住行数上限风险；畸形值 apply-time 回退引擎默认）。
- 文件系统函数在 LLM SQL 中不可用——所有数据接入必须经工具 attach 流程，LLM 不能自取数据。
- 语句超时不做：引擎无原生 per-statement 超时，自制超时必须与用户取消（ADR-0021）共用同一 interrupt 槽，且被打断语句的现行错误分类落 Runtime（可重试），语义错位——风险不抵边际收益。超时保护由资源上限（内存/行数）与用户取消承担；超时字段已随机制一并退役（残留键 parse 忽略，`retry_budget` 同款先例）。
