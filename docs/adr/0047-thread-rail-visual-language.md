# thread rail 视觉语言：单行逐字卡片 + 四类 outcome 编码 + 源事件标记与 stale 因果染色

> 部分被 [ADR-0083](./0083-conversation-stream-workspace-panel.md) 取代：Decision「（1）轮次卡片（单行）」卡片结构按对话流重写（提问 + 可折叠执行轨迹 + 答复 + 结果卡片）；「逐字提问是身份句柄、非 LLM 摘要」「（2）四类 outcome 视觉语言」（result 含义按 ADR-0077 演进）与「（3）源生命周期事件标记」（扩技能生命周期事件）、stale 因果染色保留。详见 ADR-0083。
...
**（3）源生命周期事件标记（第二物种）+ stale 因果染色**
- 标记与卡片**视觉异种**（无 outcome 图标、无提问），占时序位、恒可见（ADR-0040）：`加源 ＋` / `换源 ↻ ·失效 N 个派生`（0025）/ `删源 − ·失效 N 个派生`（0013 软删）。形态**节点化**：圆形节点（kind 字形入节点、动词右置；三档 tone 由节点 1px 边界承载，档位映射不变），连续生命周期事件 run≥2 相邻节点连线、单个孤立事件只留节点无线；turn 不上线、不缩进（技能生命周期事件同形，见 ADR-0086）。
- stale 结果卡 = **鬼影化**（降不透明度 + `●`→`⊘`）+ 内联 chip `失效·换源 X` / `失效·上游删除`；**点 chip 跳选**对应源事件标记；**不画连线**——划界：仅指 *stale 卡 ↔ 源事件* 连线；run 内生命周期连线相邻、不交错，不在本否决范围。
...
ADR-0045 定了 thread rail 为 shell 左栏、承载「轮次卡片 + 源事件标记」双物种，但卡片长相、outcome 如何区分、源事件与 stale 的因果如何编码均留为 UX 子决策。多条 ADR 的 UI 落点依赖这套视觉语言：0039（逐字标签）、0028（四类 outcome 可区分渲染）、0010/0016（assumption 归结果展示）、0017（诚实拒绝不能像失败）、0040（源事件恒可见且非轮次）、0013/0025/0041（stale 可见 + 措辞诚实）。参照系 Chat 风格 codex agent（如 ChatGPT）的紧凑卡片用 LLM 摘要，与本项目 0039 逐字约束冲突——须偏离。
...
6. **鬼影化**让「新鲜血缘 vs 死亡过去」一扫可辨；**chip-trace 承载因果不画线**——「不画线」仅指 stale 卡 ↔ 源事件连线（交错穿插下打结、渲染脆弱、违 KISS）；run 内生命周期连线相邻不交错，不在此论证范围。
...
- **画连线**连源事件与 stale 卡：交错下打结、渲染脆弱。**否决**（划界：否决仅指此类 stale 卡 ↔ 源事件连线；相邻、不交错的 run 内生命周期连线不在否决范围）。
...
- **rail 实现**：轮次卡（单行 + outcome 字形 + 条件 active chip）+ 源事件标记（异种）+ stale 鬼影 + chip-trace + stage-stale 披露横幅。
...
- **stale chip 跳选源事件标记的映射规则**：`StaleAnchor`（descriptor 内自包含因果快照：`reference_name` + `display_name` + `reason`）**无事件锚**；chip 跳选 = 「thread 中该 result 的 Turn **之后**、`reference_name` + `reason` 匹配的**最近一个** `SourceLifecycleEvent`」。因果方向保证时序（result 被某源事件失效，该事件必在其 Turn 之后）；「最近一个」消除同源多次生命周期的匹配歧义；**不加 `event_id`** 守 YAGNI（动 Rust `StaleAnchor` 契约 + recipe 序列化的不对称代价，与 ADR-0051「不加 turn id」同款论证），出口保留。v1 瑕疵：resume 后同源再次替换，「最近一个」会跳到新事件而非最初失效者（极边角、同类同源 `display_name` 多半相同），v2 加 `event_id` 出口。
