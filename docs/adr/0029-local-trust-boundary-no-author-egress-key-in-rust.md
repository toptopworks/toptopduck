...
- **被 ADR-0034 精确化（不变量 2）**：本 ADR 不变量 2「默认零用户数据持久落盘」精确化——持久 at-rest 现在存在，但**仅限用户拥有的 recipe（.duck）：本地、用户可见、无密钥**（ADR-0036 secrets-never）；本 ADR 真实隐私保证（对作者零外发、无隐藏落盘、key-in-Rust）全部保留。local-first 下「零 at-rest」是临时态期表达，持久化（0034）落地后让位于「用户拥有的 at-rest」。不变量 1（零作者外发）与不变量 3（key 仅 Rust）不受影响，0036 并将 key 隔离延伸至「key 绝不进 .duck」。
- **被 ADR-0038 校准（不变量 2）**：at-rest 进一步劈为**两类**——用户拥有的 `.duck`（0034/0036）+ app 级配置（ADR-0038，偏好/默认/窗口几何/最近文件，住 OS app-data）。「零用户数据持久落盘」精确化为「用户数据**值**只能进 `.duck` 或不落盘」，**不**禁止 app-config 存偏好/路径指针/默认参数；key 隔离（不变量 3）延伸至 app-config（key 绝不进 config）。
