# LLM output contract: one SQL + optional viz spec per turn

> 本 ADR 的「每轮一条 SQL + 可选 viz 规格」「不做单轮 pipeline」契约**被 ADR-0077 取代**——轮次 = 0 或多次工具调用，中间结果仅由显式物化调用产出（探索查询 scratch 轮末即弃）；本 ADR「文本响应分支（越界拒绝 / 消歧澄清）」「跨轮物化链式」语义**保留**。见 ADR-0077。

## Decision

每个**提问**（一轮）模型返回**一条 SQL + 可选 viz 规格**（产出**一个 `result_N`**），**或一条文本响应（不产 SQL）**——用于越界拒绝（ADR-0017）与消歧澄清（ADR-0018）。多步需求靠**跨轮物化链式**（引用前序 `result_N`）完成。**不做**单轮 pipeline，**不做** plan-then-execute。

## Context

需把 ADR-0003（物化链式）、ADR-0006/0007（viz 规格、Provider 契约）钉到**一张精确输出契约**上。

## Why

1. **KISS + YAGNI**：契约最简 → 护栏最简、错误最干净（一条查询失败 = 一个明确错误，回喂重试），UX 即 notebook cell。
2. **物化链式（ADR-0003）已让多步自然跨轮完成**，无需在单轮塞多语句。
3. **可追溯最强**：每步独立、可检视、可回退，而非一坨黑盒多步。

## Considered options

- **Pipeline**（一轮返回有序多条 SQL，各产 `result_N` + 终态 viz）：一句话做多步，但契约/错误恢复复杂（中间步失败如何回滚？viz 指哪个结果？）。**否决**。
- **Plan-then-execute**（先返回自然语言计划再生成 SQL）：透明可纠偏，但多一轮往返、延迟与成本翻倍。**否决**。

## Consequences

- 契约 = `{ sql: string, viz?: spec, assumption?: string }` | 文本态（澄清/拒绝，无 SQL）；`viz` 缺省即表格。
- `assumption?` 承载"本次 SQL 的自然语言旁注"——0018 的歧义假设标注、0017 的方法名标注（如"用了简单线性回归"）走同一字段；有 SQL 时可选附，文本态不带。结果展示/历史视图渲染为可纠偏旁注（0016/0010）。
- 复杂需求需用户多打几轮（依赖物化链式）。
- 错误恢复简单：单查询失败 → 回喂 LLM 重试 1–2 次 → 仍失败如实告知用户。
- "一轮一条 SQL"延伸为**一时刻一条查询**（ADR-0021）：查询进行时禁用输入/入队，不支持并发。
- **被 ADR-0028 校准/延伸**：契约的「失败路径」（畸形输出契约违反）明确为「失败轮」单预算重试回路，耗尽落 failure-outcome；本 ADR「单查询失败回喂重试 1–2 次」被吸收进 ADR-0028 的统一单预算（与执行逻辑错共用）。
- **被 ADR-0033 校准**：`viz?` 字段填充语义明确为「LLM 按查询意图自决是否 / 填哪种（白名单内）；用户可自然语言覆盖（如「画个柱状图」/「就显示表格」）；不设 viz 切换按钮」。
- **被 ADR-0048 延伸**：`assumption` 字段的**纠偏交互**定为「点旁注 → 聚焦 QuestionBar → 作新轮提交 → 产新 `result_N`、原轮不动」（守 0010 分枝 + 0041 不复活）。见 ADR-0048。
- **校准**：Materialized 轮的**终止文本**（agent 循环收敛时的正文回答）由 outcome 的 `body` 字段承载，走与文本态 body 相同的 markdown 渲染管线；`assumption` 保持一句话旁注语义，agent 契约下无来源、恒空（字段保留，留给未来 provider）。此前终止文本曾存于 `assumption` 槽——该姿势继承自 provider JSON 契约时代「终止文本即短注记」的形态，在旁注渲染面（纯文本槽）显示为未渲染的 markdown 原文。
