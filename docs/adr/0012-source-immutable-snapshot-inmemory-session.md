...
- **被 ADR-0029 精确化/延伸**：本 ADR L5 已允许的「透明 temp-spill」隐私框架精确化为「每会话独立临时目录 + 会话结束清除 + UI 披露」；「纯内存」承诺措辞校准为「不持久落盘 + 会话结束清除」（非零磁盘写入）；并补 minidump（本地不外发）/ 诊断日志（默认脱敏）两条本 ADR 未涉规则。
- **被 ADR-0034 取代（持久化部分）**：本 ADR「v1 不做会话持久化 / 关即重置」及 Considered options「copy-in 到持久 .duckdb 文件（会话恢复）…否决（v1）」被反转——会话现以 recipe 持久化、重启按 recipe 重读源文件**重建**本 ADR 定义的 copy-in 不可变快照。厘清：0034 的 `.duck` 是 recipe 文本文档，**非**本 ADR 否决的 `.duckdb` 数据快照库。保留：copy-in 冻结、源只读、类型推断不可逆（0004）等运行态保证不变——resume 只是重新创建运行态快照。
- **被 ADR-0042 延伸**：本 ADR copy-in 快照的 resume 重建路径钉死 = 读源文件 → 应用 recipe 的**规整参数**（用户决策）+ 当前规整算法 → 产出快照 → 校验**规整后快照 hash** == recipe 记录（不符走 ADR-0035 漂移降级）。见 ADR-0042。
