...
- **被 ADR-0108 校准**：技能声明面扩展 CLI 工具引用——与 MCP server 引用对称的声明性元数据，不注册、不启用。本 ADR 对「ad-hoc tool schema」的否决不受影响：CLI 指向用户已注册的工具，不自带可执行面。
...
- **被 ADR-0119 校准**：技能出处以**轮次调用记录**承载（name + 正文 + 发起方 + `content_hash`，随轮次持久化）；`RecipeEntry::Skill` 事件变体由其继任——新语义不再产生技能事件，旧信道并存期继续产生与折叠、随旧信道全链退役。本 ADR 的身份（`name` 即身份）、`content_hash`、诚实降级与正文点名工具引用纪律不变；`TurnProvenance.skills` 读作该轮调用记录的名字集。
