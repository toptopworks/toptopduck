...
- **`WindowControls` 组件平台分发**：
  - **macOS → `MacOSWindowControls`**（左侧）：三色圆点（red close / yellow minimize / green zoom），常显 × / − / + glyph（opacity-60 基线 + `group-hover` 提至 opacity-100 增强），click → `getCurrentWindow().{close,minimize,toggleMaximize}()`。常显满足 WCAG 1.4.1（颜色非唯一区分手段）；`<button>` 点击区域 24×24（h-6 w-6），视觉圆点 12px 作为 span 子元素，满足 WCAG 2.2 SC 2.5.8 最低目标尺寸。**省略**失焦灰显 + Alt+click（留后续）。
...
6. **保真度 F2（三色 + 常显 glyph，省略灰显 + Alt）**：glyph 常显同时满足 WCAG 1.4.1（颜色非唯一区分手段）与「非技术用户点关闭前需可见 affordance」（原生 macOS hover-only 在触摸 / 低视力 / 色盲场景失效）；`group-hover` 提至 opacity-100 保留 macOS 风格的 hover 反馈层次。失焦灰显对单窗应用（ADR-0046）价值低（unfocused 态罕见）；Alt+click 是 power-user（YAGNI）。F2 是 80/20 切。
...
- **保真度 F1（纯色点无 glyph）/ F3（含失焦灰显 + Alt+click）**：前者违 WCAG 1.4.1（颜色唯一区分手段）且背叛路线 1「可接受模拟」前提；后者失焦灰显对单窗应用价值低、Alt+click power-user YAGNI、引入命令分发抽象比直调 `getCurrentWindow` 重。**否决**——F2 是 80/20 切（常显 glyph 兼顾 a11y 与 affordance，纯 CSS `opacity` + `group-hover`），F2 → F3 是增量不重构。
...
- **新文件**：`src/shell/WindowControls.tsx`（平台分发入口）+ `MacOSWindowControls.tsx` + `WindowsWindowControls.tsx`（现 `WindowControls.tsx` 的 Windows 逻辑抽出）+ `src/shell/window-actions.ts`（共享 `fireWindowAction` helper + `WindowAction` 类型；close 失败 `log.error` / minimize / toggleMaximize 失败 `log.warn`）+ `src/shell/use-platform.ts`（`platform()` + 模块缓存 + jsdom fallback）。
...
- **未决（留实现期）**：红绿灯精确 offset / glyph 字形精细度；是否升级 F3（失焦灰显 + Alt+click）以回应 macOS 用户手感反馈；是否 supersede 路线 2（原生 overlay）若 macOS 成主力发布平台。
