...
- **校准（bar overlay 化）**：bar 的布局语义从「与 pane host 相邻的布局条带」改为「浮于主区域之上的 absolute 浮层」——rail 滚动容器全高延伸至窗口底缘（滚动条贴 app 底缘），thread 尾部内容从 bar 背后滚过，rail 底部以 bar 实测高度（ResizeObserver 发布 `--shell-bar-h` 于 `.main-area`）的动态 padding 保滚到底时末元素完整可见。Decision 1 的转场机制相应精确化：双锚点插值（centered 态 `bottom:50%` + `translateY(50%)` ↔ bottom 态 `bottom:0` + `translateY(0)`，280ms 同曲线，轨迹精确线性），pane host 高度在滑轨中恒定，`.cold-start-mode .session-pane-host { flex: 0 }` 退役。strip 交互模型：会话列由不透明 backdrop 遮蔽（右缘透口仅剩滚动条本身——宽度启动时一次性实测，overflow 出阅读列的内容画进 rail padding 也不漏），bar 底态整体 `pointer-events: none` 使透口内的滚动条可直接拖拽——卡片、backdrop、workspace 列填充重启 pointer-events，滚轮转发经事件冒泡保留。
- **校准（workspace 列退出 strip）**：上条校准引入的 workspace 列填充（`.shell-bar-track` 的 `::before`，col 3）退役——strip 在 workspace 列上方透明且不参与命中，workspace 列的内容、滚动条与 sticky 分页条直达 app 底缘，可见可点；该列上的滚轮原生滚动 workspace，不经 bar 的转发。重启 pointer-events 的面收敛为卡片与 backdrop 两面。
- **被 ADR-0118 校准**：冷启动 bar 的 pending 技能挂载流程退役——启用全集自动种子后，新会话起步即全量可发现，pending 挂载列表失去存在理由；bar 的 pending 运行时 / 模型姿势通道不受影响。
