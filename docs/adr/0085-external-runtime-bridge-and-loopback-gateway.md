...
- **外部运行时 trace 合并去重**：外部运行时 trace 双源——网关 `tools/call` 派发记录（经网关工具，权威源）+ 引擎回声行（ACP pump `session/update`、codex `mcp_tool_call`、claude `tool_use`，三路径同一合并语义）；合并规则 = 网关 trace 置前 + 引擎回声行按可归账性抵扣：builtin 名无条件丢弃；其余名按 per-name 网关配额抵扣（名下无网关行 → 保留，配额尽 → 保留并告警）；`mcp_invoke` 回声按本轮 namespaced 网关行总数计数池抵扣，不分 handle、不分结果（网关 fall-through 先解析换名再记权威行、不产生 `mcp_invoke` 壳行，见 ADR-0105 Decision 4）；经网关工具取网关记录，引擎自带工具取引擎 trace；promotions 单源网关（AcpEngine 不产 promotions）。已知边界：CLI 可能为 MCP 工具名加前缀（如 `mcp__<server>__explore`）使按名配对漏过 → 经网关工具重复；网关侧命名空间化已固化（见 ADR-0076 Decision），跨 CLI 的前缀叠加规范化留后续工作。
