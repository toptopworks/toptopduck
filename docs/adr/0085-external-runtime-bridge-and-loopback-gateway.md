...
- **网关 serve 对握手后断连免疫**：桥接进程随 turn 的外部 CLI 一同退出，硬死亡（进程退出/被杀）经 TCP reset 关闭（Windows 表现为 os error 10054），reset 毫秒级到达、几乎必然先于 engine_done 轮询命中。握手成功后的 pump 阶段，读侧 reset 与写侧 reset / broken pipe 视同桥接断开（与 EOF 同类）：serve 返回已收集的 `Ok(outcome)`，终止权保持单源归 ACP 引擎（与 cancel / engine_done 同一处置）；保留为真错误：握手失败与协议违规（over-long frame 的 `InvalidData`）。网关不依赖桥接的死亡方式判定 turn 生死（桥接 half-close 取舍不动）。
