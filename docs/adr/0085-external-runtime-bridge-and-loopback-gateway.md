...
- **外部运行时 trace 合并去重**：外部运行时 trace 双源——网关 `tools/call` 派发记录（经网关工具，权威源）+ 引擎回声行（ACP pump `session/update`、codex `mcp_tool_call`、claude `tool_use`，三路径同一合并语义）；合并规则 = 网关权威行就地替换所属轮内的引擎回声行：整行取网关值、位置留在引擎轮，结算读序保持轮内 thinking → prose → 调用，无网关置顶轮；配对 = 全部网关行按名入队 FIFO 消耗（builtin 同权、无特殊臂），`mcp_invoke` 回声不按名、按本轮 namespaced 网关行池 FIFO 抵扣，不分 handle、不分结果（网关 fall-through 先解析换名再记权威行、不产生 `mcp_invoke` 壳行，见 ADR-0105 Decision 4）；名下无网关行 → 静默保留（引擎自带工具），配额尽 → 保留并告警；无锚点的网关残余行尾随一个 flat round 兜底，审计面从不低报；promotions 单源网关（AcpEngine 不产 promotions）。已知边界：CLI 可能为 MCP 工具名加前缀（如 `mcp__<server>__explore`）使按名配对漏过 → 经网关工具重复；网关侧命名空间化已固化（见 ADR-0076 Decision），跨 CLI 的前缀叠加规范化留后续工作。
...
- **网关 serve 对握手后断连免疫**：桥接进程随 turn 的外部 CLI 一同退出，硬死亡（进程退出/被杀）经 TCP reset 关闭（Windows 表现为 os error 10054），reset 毫秒级到达、几乎必然先于 engine_done 轮询命中。握手成功后的 pump 阶段，reset / broken pipe / aborted（Windows 10053 本地栈中止变体）经共享谓词在读写两侧统一视同桥接断开（与 EOF 同类）：serve 返回已收集的 `Ok(outcome)`，终止权保持单源归 ACP 引擎（与 cancel / engine_done 同一处置）；除上述豁免外其余 io 错误一律保留 `Err`（含握手失败与协议违规——over-long frame 的 `InvalidData`，以及未列举类型）。网关不依赖桥接的死亡方式判定 turn 生死（桥接 half-close 取舍不动）。校准：pre-auth 握手读窗口同受终止旗标管辖——读超时重试并每轮先复检 cancel/engine_done；旗标已置位后到达的非超时读错误（如 teardown 的 reset 竞速唤醒 park 中的读）复检命中即返回空 outcome 交 ACP 终结，豁免于前句的握手失败保留 `Err`；干净 EOF 同受旗标复检（POSIX teardown killpg 优雅 FIN 唤醒 park 读的形态，对应 Windows 侧的 reset）——旗标已置位即交 ACP 终结；无旗标硬死与 token 不匹配 / over-long / 无旗标 EOF 照旧浮出；非 UTF-8 解码失败（InvalidData）按类归非超时读错误、随复检臂处置。
