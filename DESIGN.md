...
When the workspace folds, the conversation column promotes to primary surface — the rail scroll container spans the pane's full width (scrollbar pinned to the window edge; both flanks are rail surface and wheel-scroll), while the thread content caps to a centered reading column of `800px` minus the rail's horizontal padding inside it. The shell-level question bar overlays the scroll container's lower strip as a floating layer (ADR-0092): the rail runs the pane's full height so its scrollbar reaches the window's bottom edge, and the thread tail scrolls behind the bar. An opaque backdrop spans the conversation column up to the rail's scrollbar — its width measured once at startup, so overflow into the rail's padding stays covered too — keeping the scrollbar visible and, via a pointer-events passthrough on the strip, draggable, while a dynamic bottom padding sized from the bar's measured height keeps the last element readable at max scroll (issue #836). The bar forwards its wheel to the rail too — the whole bottom strip scrolls the conversation (issue #834).
