...
describe("ComposerSkillsSection cold-start auto-included display (#677 / #716 review tail)", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.mocked(listSkills).mockResolvedValue({
      skills: [
  ...
  it("renders an enabled builtin-with-CLI skill checked and disabled on the cold-start bar", async () => {
    renderSection(
      <ComposerSkillsSection
        sessionId={null}
        loading={false}
        ...
        pendingSkills={[]}
        cliTools={[
          { name: "python", source: "builtin", enabled: true } as never,
        ]}
      />,
    );
    const box = await screen.findByRole("checkbox", {
      name: "Mount skill python",
    });
    // The backend folds the auto-included builtin into every new session:
    // checked (agrees with the trigger count) and not toggleable off.
