...
// (ADR-0069, issue #226). These tests cover what the prior describeReject test
// held (message/detail/kind passthrough + empty-fallback) AND the concerns the
// shim hid behind it: the verb-prefix locale consistency (issue #139), the
// refreshFailed prefix, and the shell/read bare output.
...
// fallback applies only to these kinds, mirroring the prior describeReject).
describe("toAppError (shell/read bare output)", () => {
  ...
  it("falls back to the Engine locale message when fmtError yields empty", () => {
    ...
    // empty string via the `e instanceof Error` branch, so toAppError
    // substitutes the Engine locale message on shell/read so the banner is
    // never blank.
    const out = toAppError(new Error(""), intl, "shell");
    expect(out.message).toBe("Internal error");
...
// composes them with the kind-driven prefix. This pins the passthrough the
// prior describeReject test held (issue #194).
