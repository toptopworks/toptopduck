...
// invalidates the thread, so the cache always mirrors the latest settled
// turn). The pure predicate is shared with Thread's failed-card default-open
// fold: one "latest settled turn is Failed" semantic, two surfaces.
...
// timeline from the tail and read the FIRST turn entry -- source / skill
// lifecycle events never settle, so they never displace the verdict. An
// empty timeline (or one with no turns at all) reads as not-failed: nothing
// has settled, so nothing is wrong.
...
export function useTurnFailureSids(
  ...
  // append, a resume's fresh read, a close's removeQueries -- so the derive
  // below stays live against exactly what the panes see.
